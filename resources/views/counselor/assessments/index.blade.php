...
@section('content')
                        ...
                        <div class="col-12 col-sm-6 col-md-4">
                            <div class="dashboard-stat-card">
                                ...
                                <div class="stat-content">
                                    ...
                                    <div class="stat-value">{{ number_format($dassAssessments->count()) }}</div>
                                ...
                                <div class="stat-content">
                                    ...
                                    <div class="averages-grid">
                                        <div class="avg-item" title="Depression">
                                            <div class="avg-val dep">{{ number_format($avgDepression, 1) }}</div>
                                            <div class="avg-label">Dep</div>
                                        </div>
                                        <div class="avg-item" title="Anxiety">
                                            <div class="avg-val anx">{{ number_format($avgAnxiety, 1) }}</div>
                                            <div class="avg-label">Anx</div>
                                        </div>
                                        <div class="avg-item" title="Stress">
                                            <div class="avg-val str">{{ number_format($avgStress, 1) }}</div>
                        ...
                        <div class="card-body">
                            ...
                            <form method="GET" class="mb-3">
                                ...
                                <select name="type" class="form-select">
                                    ...
                                    <option value="GRIT Scale" @if(request('type')=='GRIT Scale') selected @endif>GRIT Scale</option>
                                    <option value="NEO-PI-R" @if(request('type')=='NEO-PI-R') selected @endif>NEO-PI-R</option>
                                    <option value="Work Values Inventory" @if(request('type')=='Work Values Inventory') selected @endif>Work Values Inventory</option>
                                </select>
                            </div>
                                    <div class="col-md-2">
                                        ...
                                        <select name="college" class="form-select">
                                            ...
                                            @foreach($colleges as $col)
                                                <option value="{{ $col }}" @if(request('college')==$col) selected @endif>{{ $col }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                                        ...
                                        <select name="year" class="form-select">
                                            ...
                                            <option value="1st Year" @if(request('year') == '1st Year') selected @endif>1st
                                                Year</option>
                                            <option value="2nd Year" @if(request('year') == '2nd Year') selected @endif>2nd
                                                Year</option>
                                            <option value="3rd Year" @if(request('year') == '3rd Year') selected @endif>3rd
                                                Year</option>
                                            <option value="4th Year" @if(request('year') == '4th Year') selected @endif>4th
                                                Year</option>
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                                        ...
                                        <select name="gender" class="form-select">
                                            ...
                                            <option value="Female" @if(request('gender') == 'Female') selected @endif>Female
                                            </option>
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <input type="text" name="student" class="form-control"
                                            value="{{ request('student') }}" placeholder="Search...">
                                    </div>
                                        ...
                                        @forelse($assessments as $assessment)
                                            ...
                                            <tr class="align-middle">
                                                <td class="text-center align-middle"><span
                                                        class="badge rounded-pill bg-light text-dark border border-1">{{ $assessment->type }}</span>
                                                </td>
                                                <td class="text-center align-middle" style="white-space:nowrap;">
                                                    {{ $assessment->created_at->format('M d, Y h:i A') }}</td>
                                                <td class="text-start align-middle">
                                                    {{ $assessment->user->name ?? 'N/A' }}
                                                </td>
                                                <td class="text-start align-middle"
                                                    style="max-width: 240px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;"
                                                    title="{{ $assessment->user->email ?? 'N/A' }}">
                                                    {{ $assessment->user->email ?? 'N/A' }}</td>
                                                <!-- Risk indicator removed -->
                                                @if($assessment->type === 'DASS-42')
                                                    <td class="text-end align-middle"><span
                                                            class="badge bg-primary">{{ $depTotal ?? ($scores['depression'] ?? '-') }}</span>
                                                    </td>
                                                    <td class="text-end align-middle"><span
                                                            class="badge bg-info text-dark">{{ $anxTotal ?? ($scores['anxiety'] ?? '-') }}</span>
                                                    </td>
                                                    <td class="text-end align-middle"><span
                                                            class="badge bg-secondary">{{ $strTotal ?? ($scores['stress'] ?? '-') }}</span>
                                                    </td>
                                                @else
    ...
    @foreach($assessments as $assessment)
        ...
        <!-- View Summary Modal -->
        <div class="modal fade" id="summaryModal-{{ $assessment->id }}" tabindex="-1"
            aria-labelledby="summaryModalLabel-{{ $assessment->id }}" aria-hidden="true">
