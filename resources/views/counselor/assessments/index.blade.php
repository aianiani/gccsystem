            ...
            <div class="main-dashboard-content flex-grow-1">
                <div class="main-dashboard-inner">
                        ...
                        <div class="stat-card">
                            ...
                            <div class="stat-info">
                                <h3 style="font-size: 1.2rem; line-height: 1.2;">
                                    <span class="text-primary">{{ number_format($avgDep, 1) }}</span> /
                                    <span class="text-success">{{ number_format($avgAnx, 1) }}</span> /
                                    <span class="text-danger">{{ number_format($avgStr, 1) }}</span>
                                </h3>
                    ...
                    <div class="filter-bar">
                        <form method="GET" action="{{ route('counselor.assessments.index') }}">
                            <div class="filter-row">
                                ...
                                <div class="filter-input-group">
                                    ...
                                    <select name="type" class="filter-control">
                                        ...
                                        <option value="DASS-42" {{ request('type') == 'DASS-42' ? 'selected' : '' }}>DASS-42
                                        </option>
                                        <option value="GRIT Scale" {{ request('type') == 'GRIT Scale' ? 'selected' : '' }}>
                                            GRIT Scale</option>
                                        <option value="Personality (NEO-FFI)" {{ request('type') == 'Personality (NEO-FFI)' ? 'selected' : '' }}>NEO-FFI</option>
                                        <option value="Work Values Inventory" {{ request('type') == 'Work Values Inventory' ? 'selected' : '' }}>WVI</option>
                                    </select>
                                </div>
                                ...
                                <div class="filter-input-group">
                                    ...
                                    <select name="college" class="filter-control">
                                        ...
                                        @php $colleges = ['CAS', 'CVM', 'CFES', 'CBM', 'CON', 'CHE', 'CA', 'CISC', 'CED', 'COE']; @endphp
                                        @foreach($colleges as $col)
                                            <option value="{{ $col }}" {{ request('college') == $col ? 'selected' : '' }}>
                                                {{ $col }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="filter-input-group">
                                    ...
                                    <select name="sort_by" class="filter-select">
                                        ...
                                        <option value="oldest" {{ request('sort_by') == 'oldest' ? 'selected' : '' }}>Oldest
                                            First</option>
                                        <option value="name_az" {{ request('sort_by') == 'name_az' ? 'selected' : '' }}>Name
                                            A-Z</option>
                                        <option value="name_za" {{ request('sort_by') == 'name_za' ? 'selected' : '' }}>Name
                                            Z-A</option>
                                    </select>
                                </div>
                                ...
                                <div class="filter-input-group">
                                    ...
                                    <select name="per_page" class="filter-select" style="min-width: 100px;">
                                        ...
                                        <option value="30" {{ request('per_page') == '30' ? 'selected' : '' }}>30 Per Page
                                        </option>
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <a href="{{ route('counselor.assessments.index') }}" class="btn-reset-filter"
                                        title="Reset">
                    ...
                    <div class="content-card">
                        ...
                        <form id="bulk-delete-form" action="{{ route('counselor.assessments.bulkDestroy') }}" method="POST"
                            class="d-none">
                            @csrf @method('DELETE')
                        </form>
                        ...
                        @if($assessments->count() > 0)
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="user-info-cell">
                                                        <img src="{{ $assessment->user->avatar_url }}" class="user-avatar" alt="">
                                                        <div class="user-details">
                                                            <h6>{{ $assessment->user->name }}</h6>
                                                            <span>{{ $assessment->user->student_id ?? '-' }} •
                                                                {{ $assessment->user->college ?? '' }}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <div class="d-flex flex-column">
                                                        <span
                                                            class="fw-semibold">{{ $assessment->created_at->format('M d, Y') }}</span>
                                                        <span
                                                            class="text-muted small">{{ $assessment->created_at->format('g:i A') }}</span>
                                                    </div>
                                                </td>
                                                <td>
                                                    @if($assessment->type === 'DASS-42')
                                                        @php $s = is_array($assessment->score) ? $assessment->score : json_decode($assessment->score, true); @endphp
                                                        <span class="score-badge score-dep"
                                                            title="Depression">{{ $s['depression'] ?? 0 }}</span>
                                                        <span class="score-badge score-anx"
                                                            title="Anxiety">{{ $s['anxiety'] ?? 0 }}</span>
                                                        <span class="score-badge score-str"
                                                            title="Stress">{{ $s['stress'] ?? 0 }}</span>
                                                    @else
                                                ...
                                                <td>
                                                    <div class="actions-cell">
                                                        <a href="{{ route('counselor.assessments.show', $assessment->id) }}"
                                                        ...
                                                        <form id="delete-form-{{ $assessment->id }}"
                                                            action="{{ route('counselor.assessments.destroy', $assessment->id) }}"
                            ...
                            <div class="pagination-wrap">
                                {{ $assessments->links('vendor.pagination.premium') }}
                            </div>
                        @else
