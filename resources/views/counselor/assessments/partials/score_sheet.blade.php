            ...
            <table class="table table-hover table-bordered align-middle">
                    ...
                    @foreach($grit_questions as $idx => $q)
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <span class="badge {{ $isPassion ? 'bg-warning text-dark' : 'bg-info text-dark' }}">
                                    {{ $isPassion ? 'Passion' : 'Perseverance' }}
                                </span>
                            </td>
                            <td class="text-center fw-bold text-success">{{ $gritPoint ?: '-' }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot class="table-light fw-bold text-center">
                    <tr>
                        ...
                        <td class="text-warning">{{ $rawScores['passion_index'] ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td colspan="4" class="text-end px-4">Perseverance Score Index (Avg):</td>
                        <td class="text-info">{{ $rawScores['perseverance_index'] ?? '-' }}</td>
                    </tr>
                    <tr class="table-success">
                        <td colspan="4" class="text-end px-4" style="font-size: 1.1rem;">TOTAL GRIT SCORE INDEX:</td>
                        <td style="font-size: 1.1rem; color: #1f7a2d;">{{ $rawScores['total_index'] ?? '-' }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
    ...
    <div class="score-sheet-block">
        ...
        <div class="row g-3 mb-5">
            @foreach($domainConfigs as $name => $conf)
                <div class="col-md-4">
                    <div class="card h-100 shadow-sm border-0" style="border-left: 5px solid {{ $conf['color'] }} !important;">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <span class="fw-bold" style="color: {{ $conf['color'] }}">{{ strtoupper($name) }}</span>
                                <i class="bi {{ $conf['icon'] }} fs-4" style="color: {{ $conf['color'] }}"></i>
                            </div>
                            <div class="h3 mb-0 fw-bold">{{ $domains[$name] ?? 0 }}</div>
                            <div class="progress mt-2" style="height: 6px;">
                                ...
                                <div class="progress-bar" role="progressbar"
                                    style="width:{{ $percent }}%; background-color: {{ $conf['color'] }}"></div>
                            </div>
                ...
                <table class="table table-sm table-bordered mb-0" style="font-size: 0.75rem;">
                    ...
                    <tbody>
                        <tr class="text-center">
                            <td>Neuroticism</td>
                            <td>35.32</td>
                            <td>{{ $domains['Neuroticism'] ?? '-' }}</td>
                        </tr>
                        <tr class="text-center">
                            <td>Extroversion</td>
                            <td>35.85</td>
                            <td>{{ $domains['Extroversion'] ?? '-' }}</td>
                        </tr>
                        <tr class="text-center">
                            <td>Openness</td>
                            <td>29.55</td>
                            <td>{{ $domains['Openness'] ?? '-' }}</td>
                        </tr>
                        <tr class="text-center">
                            <td>Agreeableness</td>
                            <td>44.50</td>
                            <td>{{ $domains['Agreeableness'] ?? '-' }}</td>
                        </tr>
                        <tr class="text-center">
                            <td>Conscientiousness</td>
                            <td>38.01</td>
                            <td>{{ $domains['Conscientiousness'] ?? '-' }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
                ...
                <tbody>
                    @foreach($neo_questions as $idx => $q)
                        ...
                        <tr>
                            ...
                            <td>{{ $q }} @if($isNeg)<span class="text-muted small">(R)</span>@endif</td>
                            <td class="text-center fw-bold">{{ $answers[$idx] ?? '-' }}</td>
                            <td class="text-center small">
                                <span class="badge" style="background-color: {{ $color }}">{{ substr($domName, 0, 1) }}</span>
                                {{ $domName }}
                            </td>
                        </tr>
                    @endforeach
    ...
    <div class="score-sheet-block">
        ...
        <div class="row g-3 mb-5">
            @foreach($wviConfigs as $name => $items)
                ...
                @endphp
                <div class="col-md-4">
                    <div class="card h-100 shadow-sm border-0" style="border-top: 4px solid {{ $inter['color'] }} !important;">
                        <div class="card-body p-3">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <span class="fw-bold small text-muted text-uppercase">{{ $name }}</span>
                                <span class="badge" style="background-color: {{ $inter['color'] }}; font-size: 0.65rem;">{{ $inter['label'] }}</span>
                            </div>
                            <div class="h3 mb-1 fw-bold">{{ $sum }}</div>
                            <div class="progress" style="height: 6px;">
                                <div class="progress-bar" role="progressbar" style="width:{{ ($sum / 15) * 100 }}%; background-color: {{ $inter['color'] }}"></div>
                            </div>
                            <div class="mt-2 small text-muted">Items: {{ implode(', ', $items) }}</div>
                        </div>
                    </div>
                </div>
            @endforeach
