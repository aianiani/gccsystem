        ...
        <div class="container py-4" style="max-width:1100px;">
            ...
            <div class="d-flex align-items-center justify-content-between mb-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <h2 class="fw-bold mb-0 d-flex align-items-center gap-2"><i class="bi bi-pencil"></i> Edit Session Note</h2>
                </div>
            </div>

            <style>
                :root { --primary-green: #1f7a2d; --primary-green-2: #13601f; --forest-green: var(--primary-green); }
                .session-card { border-radius: 12px; box-shadow: 0 8px 30px rgba(14,56,20,0.06); }
                .edit-hero { display: grid; grid-template-columns: 80px 1fr auto; gap: 1rem; align-items: center; background: linear-gradient(135deg, var(--primary-green) 0%, var(--primary-green-2) 100%); color: #fff; padding: 0.9rem 1rem; border-radius: 12px; }
                .edit-hero .hero-avatar { width:80px; height:80px; border-radius:50%; object-fit:cover; border:3px solid rgba(255,255,255,0.12); }
                .hero-meta { display:inline-flex; align-items:center; gap:0.6rem; background: rgba(255,255,255,0.06); padding: 0.4rem 0.6rem; border-radius:8px; }
                ...
                @media (max-width:900px) { .edit-hero { grid-template-columns: 64px 1fr; } .edit-hero .hero-right { display:none; } }
            </style>
            ...
            @if(trim($note->note) === '')
                <div class="alert alert-warning mb-3">This session note is empty. Please fill it out and save to complete the session.</div>
            @endif

            <form action="{{ route('counselor.session_notes.update', $note->id) }}" method="POST" id="editSessionNoteForm">
                @csrf
                @method('PATCH')
                <div class="row">
                    <div class="col-lg-8">
                        <div class="card session-card p-4 mb-4">
                            ...
                            <textarea name="note" id="note" class="form-control note-textarea" maxlength="4000">{{ old('note', $note->note) }}</textarea>
                            @error('note')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                            <div class="d-flex justify-content-between mt-2 align-items-center">
                                <div class="text-muted small">Be objective and concise. Sensitive details should be minimal.</div>
                                <div class="char-counter" id="charCounter">{{ strlen(old('note', $note->note) ?? '') }} / 4000</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="card session-card p-3 mb-3">
                            ...
                            <input type="datetime-local" name="next_session" id="next_session" class="form-control mb-3" value="{{ old('next_session', $note->next_session ? $note->next_session->format('Y-m-d\\TH:i') : '') }}">

                            <hr>
                            <div class="meta-label">Created</div>
                            <div class="small text-muted mb-2">{{ $note->created_at ? $note->created_at->format('F j, Y g:i A') : '-' }}</div>
                            <div class="meta-label">Last Updated</div>
                            <div class="small text-muted mb-2">{{ $note->updated_at ? $note->updated_at->diffForHumans() : '-' }}</div>
                        </div>

                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-success btn-lg"><i class="bi bi-check-circle me-1"></i> Save Changes</button>
                            <a href="{{ route('counselor.session_notes.show', $note->id) }}" class="btn btn-outline-secondary">Cancel</a>
                        </div>
                    </div>
                </div>
            </form>
