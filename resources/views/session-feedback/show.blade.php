                                    ...
                                    <div class="col-lg-5">
                                        <div class="session-summary h-100 mb-0">
                                            ...
                                            <div class="student-info">
                                                ...
                                                <div>
                                                    <h6 class="mb-0 fw-bold">{{ $feedback->appointment->student->name }}
                                                    </h6>
                                                    <p class="mb-0 text-muted small">
                                                        {{ $feedback->appointment->student->email }}</p>
                                                </div>
                                            </div>
                                            <div class="d-flex flex-column gap-2 mb-4">
                                                <p class="mb-0"><i class="bi bi-calendar3 me-2 text-primary"></i>
                                                    <strong>Date:</strong>
                                                    {{ $feedback->appointment->scheduled_at->format('M j, Y') }}</p>
                                                <p class="mb-0"><i class="bi bi-clock me-2 text-primary"></i>
                                                    <strong>Time:</strong>
                                                    {{ $feedback->appointment->scheduled_at->format('g:i A') }}</p>
                                                <p class="mb-0"><i class="bi bi-hash me-2 text-success"></i> <strong>Session
                                                        Number:</strong> {{ $sessionNumber }}</p>
                                                <p class="mb-0"><i class="bi bi-journal-text me-2 text-success"></i>
                                                    <strong>Notes:</strong>
                                                    {{ $feedback->appointment->sessionNotes->count() }} note(s)</p>
                                            </div>
                                            ...
                                            <div class="meta-info border-top pt-3 d-flex flex-column gap-2">
                                                <div class="meta-item"><i class="bi bi-calendar-event"></i> Submitted on
                                                    {{ $feedback->created_at->format('F j, Y') }}</div>
                                                <div class="meta-item"><i class="bi bi-clock-history"></i>
                                                    {{ $feedback->created_at->diffForHumans() }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-lg-7">
                                        <div class="rating-box mb-4">
                                            <div>
                                                ...
                                                <div class="stars">
                                                    @for ($i = 1; $i <= 5; $i++)
                                                        <i
                                                            class="bi bi-star{{ $i <= $feedback->rating ? '-fill' : '' }} star"></i>
                                                    @endfor
                                                </div>
                                            </div>
                                            <div class="text-end">
