            ...
            <div class="col-md-6">
                <div class="chart-card">
                    <div class="chart-title">
                        ...
                        <span class="metric-badge ms-auto"><?php echo e($demographics['college']->sum()); ?> Students</span>
                    </div>
                    <div style="height: 200px;">
            ...
            <div class="col-md-4">
                <div class="chart-card">
                    <div class="chart-title">
                        ...
                        <span class="metric-badge ms-auto"><?php echo e($assessmentTypeStats->sum()); ?> Total</span>
                    </div>
            ...
            <div class="col-12">
                <div class="chart-card">
                    <div class="chart-title">
                        ...
                        <span class="metric-badge ms-auto"><?php echo e($seminarStats->sum()); ?> Attendees</span>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            new Chart(document.getElementById('sexChart'), {
                type: 'doughnut',
                data: {
                    labels: <?php echo json_encode($demographics['sex']->keys(), 15, 512) ?>,
                    datasets: [{ data: <?php echo json_encode($demographics['sex']->values(), 15, 512) ?>, backgroundColor: [colors.primary, colors.secondary, colors.gray] }]
                },
            ...
            new Chart(document.getElementById('yearChart'), {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode($demographics['year_level']->keys()->map(fn($k) => $k . (is_numeric($k) ? 'th Year' : '')), 15, 512) ?>,
                    datasets: [{ label: 'Students', data: <?php echo json_encode($demographics['year_level']->values(), 15, 512) ?>, backgroundColor: colors.primary, borderRadius: 5 }]
                },
            ...
            new Chart(document.getElementById('collegeChart'), {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode($demographics['college']->keys(), 15, 512) ?>,
                    datasets: [{ label: 'Enrollment', data: <?php echo json_encode($demographics['college']->values(), 15, 512) ?>, backgroundColor: colors.info, borderRadius: 5 }]
                },
            ...
            new Chart(document.getElementById('counselingStatusChart'), {
                type: 'pie',
                data: {
                    labels: <?php echo json_encode($counselingStatus->keys()->map(fn($s) => ucfirst($s)), 15, 512) ?>,
                    datasets: [{ data: <?php echo json_encode($counselingStatus->values(), 15, 512) ?>, backgroundColor: colors.palette }]
                },
            ...
            new Chart(document.getElementById('counselingNatureChart'), {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode(array_keys($counselingNature), 15, 512) ?>,
                    datasets: [{ label: 'Appointments', data: <?php echo json_encode(array_values($counselingNature), 15, 512) ?>, backgroundColor: [colors.primary, colors.info, colors.secondary, colors.gray], borderRadius: 8 }]
                },
            ...
            new Chart(document.getElementById('counselorLoadChart'), {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode($counselorWorkload->keys(), 15, 512) ?>,
                    datasets: [{ label: 'Session Notes', data: <?php echo json_encode($counselorWorkload->values(), 15, 512) ?>, backgroundColor: colors.primary, borderRadius: 8 }]
                },
                options: { responsive: true, maintainAspectRatio: false, indexAxis: 'y', plugins: { legend: { display: false } } }
            });
            ...
            new Chart(document.getElementById('assessmentTypeChart'), {
                type: 'doughnut',
                data: {
                    labels: <?php echo json_encode($assessmentTypeStats->keys(), 15, 512) ?>,
                    datasets: [{ data: <?php echo json_encode($assessmentTypeStats->values(), 15, 512) ?>, backgroundColor: colors.palette }]
                },
            ...
            new Chart(document.getElementById('assessmentTrendChart'), {
                type: 'line',
                data: {
                    labels: <?php echo json_encode($assessmentTrend->keys(), 15, 512) ?>,
                    datasets: [{ label: 'Assessments Taken', data: <?php echo json_encode($assessmentTrend->values(), 15, 512) ?>, borderColor: colors.primary, backgroundColor: 'rgba(31, 122, 45, 0.1)', fill: true, tension: 0.4 }]
                },
                options: { responsive: true, maintainAspectRatio: false }
            });
            ...
            new Chart(document.getElementById('assessmentCollegeChart'), {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode($assessmentParticipation->keys(), 15, 512) ?>,
                    datasets: [{ label: 'Unique Participants', data: <?php echo json_encode($assessmentParticipation->values(), 15, 512) ?>, backgroundColor: colors.secondary, borderRadius: 5 }]
                },
                options: { responsive: true, maintainAspectRatio: false, indexAxis: 'y', plugins: { legend: { display: false } } }
            });
            ...
            new Chart(document.getElementById('seminarChart'), {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode($seminarStats->keys(), 15, 512) ?>,
                    datasets: [{ label: 'Attendees', data: <?php echo json_encode($seminarStats->values(), 15, 512) ?>, backgroundColor: colors.info, borderRadius: 10, maxBarThickness: 50 }]
                },
                options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } }
            });
        });
